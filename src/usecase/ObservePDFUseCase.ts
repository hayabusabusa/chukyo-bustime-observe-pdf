...
export class ObservePDFUseCase {
  ...
  async execute(): Promise<void> {
    // 運行カレンダーの PDF の差分を確認.
    const compareCalendarPDFPromise = this.comparePDF(URLs.pdf, "calendar");
    // 時刻表の PDF の差分を確認.
    const compareTimetablePDFPromise = this.comparePDF(URLs.timeTable, "timetable");
   ...
   * @returns 非同期処理の完了.
   */
  private async comparePDF(url: string, fileName: string): Promise<void> {
